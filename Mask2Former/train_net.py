...
from mask2former import (
    ...
    MaskFormerSemanticDatasetMapperLITS,
...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        ...
        elif cfg.INPUT.DATASET_MAPPER_NAME == "mask_former_semantic_lits":
            mapper = MaskFormerSemanticDatasetMapperLITS(cfg, True)
            return build_detection_train_loader(cfg, mapper=mapper)
    ...
    def build_test_loader(cfg, dataset_name):
        mapper = MaskFormerSemanticDatasetMapperLITS(cfg, False)
...
def get_lits_dicts(img_dir):
    ...
    for idx, v in enumerate(files):
        ...
        height, width = cv2.imread(filenameImg,-1).shape[:2]
...
def main(args):
    cfg = setup(args)
    for d in ["train","val"]:
        DatasetCatalog.register("lits_" + d, lambda d=d: get_lits_dicts("../../../Data/liver_only/full_data/" + d))
        MetadataCatalog.get("lits_" + d).set(thing_classes=[],ignore_label=2,evaluator_type="sem_seg_iou",stuff_classes=["lits"])
    print("Dataset Ready")
    if args.eval_only:
